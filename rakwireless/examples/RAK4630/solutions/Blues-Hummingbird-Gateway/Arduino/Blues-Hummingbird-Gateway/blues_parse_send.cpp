...
bool blues_parse_send(uint8_t *data, uint16_t data_len)
{
	...
	if (body != NULL)
	{
		...
		while (byte_idx < data_len)
		{
			...
			{
				// Wrong sensor ID
				Serial.printf("Unknown Sensor %d\n", data[current_byte_idx]);
				JAddStringToObject(body, "error", "Invalid LPP ID");
				JAddItemToObject(req, "body", body);
				if (!blues_send_req())
				{
					Serial.printf("Failed to send error packet\n");
				}
				return false;
			}
			Serial.printf("Found Sensor %d\n", data[current_byte_idx]);
			current_byte_idx++;
			switch (value_id[sens_idx])
			{
			case 113:
			case 134:
				Serial.printf("Found accelerometer or gyrometer\n");
				...
				Serial.printf("x %.4f y %.4f z %.4f\n", float_val1, float_val2, float_val3);
				break;
			case 136:
				Serial.printf("Found GPS 4 digit\n");
				sens_full_name = value_name[sens_idx] + "_" + String(sens_num);
				sec_lvl = JAddObjectToObject(body, sens_full_name.c_str());
				...
				Serial.printf("lat %.4f lng %.4f alt %.4f\n", float_val1, float_val2, float_val3);
				break;
			case 137:
				Serial.printf("Found GPS 6 digit\n");
				sens_full_name = value_name[sens_idx] + "_" + String(sens_num);
				sec_lvl = JAddObjectToObject(body, sens_full_name.c_str());
			...
			case 135:
				...
				Serial.printf("r %ld g %ld b %ld\n", unsigned_val1, unsigned_val2, unsigned_val3);
				break;
			case 255:
				unsigned_val1 = 0;
				...
				Serial.printf("unsigned_val1 %s\n", node_id_str);

				sens_full_name = value_name[sens_idx] + "_" + String(sens_num);
				JAddStringToObject(body, value_name[sens_idx].c_str(), node_id_str);

				Serial.printf("Added %s %08LX\n", sens_full_name.c_str(), (uint64_t)unsigned_val1);
			...
			default:
				...
				Serial.printf("Added %s %.2f\n", sens_full_name.c_str(), float_val1);

				break;
			}
			byte_idx = byte_idx + value_size[sens_idx] + 2;
			// Serial.printf("Data size %d Position %d", data_len, byte_idx);
			Serial.printf(">>>>><<<<<\n");
		}
		JAddItemToObject(req, "body", body);
